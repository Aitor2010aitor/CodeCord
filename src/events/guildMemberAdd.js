    ...
    async execute(member, client) {
        try {
            ...
            try {
                ...
                if (modConfig) {
                    // Formato del panel (Moderación -> Rol Automático al Entrar)
                    const actions = modConfig.actions || {};
                    let roleId = actions.autoRoleOnJoin || '';
                    if (!roleId) {
                        // Formato antiguo de AutoRol
                        const autorole = modConfig.autorole || {};
                        if (autorole.enabled && autorole.roleId) roleId = autorole.roleId;
                    }
                    if (roleId) {
                        ...
                        if (role) {
                            ...
                            console.error(`AutoRol: Rol ${roleId} no encontrado en ${member.guild.name}`);
                        }
                    }
                }
            } catch (err) {
