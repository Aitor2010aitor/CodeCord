...
module.exports = [
    {
        ...
        async execute(reaction, user, client) {
            ...
            try {
                ...
                if (!config || !Array.isArray(config.reactions)) return;

                const emojiIdentifier = reaction.emoji.id || reaction.emoji.name;
                const targetReaction = config.reactions.find(r => 
                    r.emoji === emojiIdentifier || 
                    r.emoji === reaction.emoji.name || 
                    (reaction.emoji.id && String(r.emoji).includes(reaction.emoji.id)) ||
                    r.emoji === reaction.emoji.toString()
                );

                if (targetReaction && targetReaction.roleId) {
                    const member = await reaction.message.guild.members.fetch(user.id).catch(() => null);
                    if (member) {
                        await member.roles.add(targetReaction.roleId).catch(err => {
                            console.error(`❌ [AutoRol] Error al asignar rol ${targetReaction.roleId} a ${user.tag}:`, err.message);
                        });
                    }
                }
            } catch (e) {
        ...
        async execute(reaction, user, client) {
            ...
            try {
                ...
                if (targetReaction && targetReaction.roleId) {
                    const member = await reaction.message.guild.members.fetch(user.id).catch(() => null);
                    if (member) {
                        await member.roles.remove(targetReaction.roleId).catch(err => {
                            console.error(`❌ [AutoRol] Error al remover rol ${targetReaction.roleId} a ${user.tag}:`, err.message);
                        });
                    }
                }
            } catch (e) {
