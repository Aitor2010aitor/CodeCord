// Archivo: src/commands/voice/nex.js
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!isLogChannel) {
            return interaction.reply({ content: '❌ Este comando solo se puede usar en el canal de logs/soporte de voz.', ephemeral: true });
        }
        ...
        if (!hasNextRole && !hasStaffRole && !interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return interaction.reply({ content: '❌ No tienes el rol de soporte de voz para atender la cola.', ephemeral: true });
        }

        const queue = client.voiceSupportQueue.get(interaction.guild.id);
        if (!queue || queue.length === 0) {
            return interaction.reply({ content: '❌ No hay usuarios en la cola de espera.', ephemeral: true });
        }

        const supportChannels = findSupportChannels(interaction.guild);
        if (supportChannels.size === 0) {
            return interaction.reply({ content: '❌ No se encontraron canales de soporte de voz.', ephemeral: true });
        }
