// Archivo: src/commands/administradores/sanctionhistory.js
...
module.exports = {
    ...
    async execute(interaction, client) {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ModerateMembers)) {
            return interaction.reply({ content: '❌ No tienes permisos para ver las sanciones.', flags: MessageFlags.Ephemeral });
        }
        ...
        if (records.length === 0) {
            return interaction.reply({ content: '📜 No se encontraron sanciones registradas.', flags: MessageFlags.Ephemeral });
        }
