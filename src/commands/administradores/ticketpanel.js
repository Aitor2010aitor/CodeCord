// Archivo: src/commands/administradores/ticketpanel.js
...
module.exports = {
    ...
    async execute(interaction, client) {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild) && !interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return interaction.reply({ content: '❌ Necesitas permisos de gestión para ejecutar este comando.', flags: MessageFlags.Ephemeral });
        }
        ...
        await interaction.reply({ content: '✅ Panel de tickets publicado exitosamente.', flags: MessageFlags.Ephemeral });
    }
};
