...
El script registra los comandos globalmente para todos los servidores donde esté instalado el bot. También elimina los comandos antiguos de los IDs definidos en `GUILD_ID`, `GUILD_ID_1`, `GUILD_ID_2` y `GUILD_ID_3` para evitar duplicados.

El registro global de Discord puede tardar hasta 1 hora en aparecer.

Scripts auxiliares en `scripts/`:

| Script | Uso |
|---|---|
| `deploy-commands.js` | Despliegue global (el de `npm run deploy`) |
