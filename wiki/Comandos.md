...
CodeCord registra **50 slash commands**. Los comandos de juegos usan el prefijo `!`; solo `/juegos` permanece como slash command de juegos.
...
Juegos disponibles: `!8ball`, `!coinflip`, `!dado`, `!rps`, `!ship` y `!trivia`.
...
| `/warn` | Aplica una advertencia formal y notifica al usuario por MD |
| `/unwar` | Retira la advertencia más reciente de un usuario |
...
Los comandos de AutoMod se configuran desde el panel web, en **Moderación / AutoMod** y **Anti-Raid**.
...
Cada comando comprueba sus propios permisos con `PermissionsBitField` (por ejemplo `/ban` exige `BanMembers` o `ModerateMembers`). Además, `/setroles` permite autorizar roles concretos para usar los comandos del bot, y `/staffrole` define los roles de staff que usan los sistemas de tickets y soporte.
