...
// client.ticketConfigData removido porque ahora la configuración es por servidor
...
// Restaurar cambios de color automáticos al reiniciar el bot (lee de servidores/)
try {
            ...
            if (match) {
                ...
                if (colorData.roleId) {
                    ...
                    if (guild) {
                        ...
                        startColorRotation(guild, colorData.speed);
    ...
    try {
        {
            ...
            const arConfig = { guilds: { [message.guild.id]: guildResponses } };
    ...
    try {
        {
            ...
            const welcomeConfig = guildConfig ? { [member.guild.id]: guildConfig } : {};
...
client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.customId.startsWith('create_ticket')) {
            ...
            const respond = async (options) => {
                try {
                    ...
                    console.error('[TICKETS] Error enviando respuesta:', e);
                }
            };

            // Cargar configuración desde el archivo JSON
            let guildConfig = getTicketConfig(interaction.guild.id);
            console.log(`[DEBUG-TICKETS] GuildID: ${interaction.guild.id}`);
            console.log(`[DEBUG-TICKETS] Config cargada de servidores:`, JSON.stringify(guildConfig, null, 2));
            console.log(`[DEBUG-TICKETS] buttonIndex buscado: ${buttonIndex}`);
            ...
            if (maxTickets > 0) {
                ...
                if (userTickets.size >= maxTickets) {
                    console.warn(`[TICKETS] El usuario ${interaction.user.tag} ya tiene el máximo de tickets abiertos (${userTickets.size}/${maxTickets}).`);
                    return respond({ content: `❌ Ya tienes el límite máximo de tickets abiertos en este servidor (${maxTickets}). Por favor, cierra tus tickets anteriores antes de abrir uno nuevo.`, flags: 64 });
                }
            }
            ...
            // Si tiene pregunta, mostramos el modal (NO podemos deferReply aquí porque un modal requiere una interacción fresca sin responder/defer)
            const hasQuestion = !!(buttonConfig && buttonConfig.question);
            console.log(`[DEBUG-TICKETS] buttonConfig encontrado:`, JSON.stringify(buttonConfig, null, 2));
            console.log(`[DEBUG-TICKETS] hasQuestion = ${hasQuestion}`);

            if (!hasQuestion) {
                try {
                    await interaction.deferReply({ flags: 64 });
                } catch (error) {
                    console.error('[TICKETS] Error al deferReply al inicio de la interacción:', error);
                }
            }
            ...
            if (!match) {
                ...
                return respond({ content: '❌ Error interno: no pude identificar el botón de ticket.', flags: 64 });
            }

            if (!buttonConfig) {
                console.warn('[TICKETS] No se encontró configuración para el botón:', interaction.customId, 'guild:', interaction.guild.id);
                return respond({ content: '❌ No pude cargar la configuración del ticket. Vuelve a crear el panel.', flags: 64 });
            }
            ...
            if (buttonConfig.question) {
                ...
                try {
                    console.log(`[DEBUG-TICKETS] Intentando mostrar modal al usuario ${interaction.user.tag}...`);
                    await interaction.showModal(modal);
                    console.log(`[DEBUG-TICKETS] Modal mostrado con éxito.`);
                } catch (error) {
                    console.error('[TICKETS] Error mostrando modal:', error);
                    return respond({ content: '❌ No pude abrir el formulario. Comprueba los permisos del bot.', flags: 64 });
                }
                return;
            }
            ...
            if (!interaction.guild.members.me.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
                ...
                return respond({ content: '❌ Necesito el permiso "Gestionar Canales" para crear tickets.', flags: 64 });
            }

            try {
                let category = interaction.guild.channels.cache.find(c =>
                    c.type === ChannelType.GuildCategory &&
                    /ticket|soporte|support/i.test(c.name) &&
                    interaction.guild.channels.cache.filter(ch => ch.parentId === c.id).size < 50
                );

                // Si no existe categoría o todas están llenas, crearla automáticamente
                if (!category) {
                    console.log(`[TICKETS] No se encontró categoría de tickets disponible (o las existentes están llenas), creando una nueva...`);
                    try {
                        ...
                        return respond({ content: '❌ Error al crear la categoría de tickets. Verifica los permisos del bot.' });
                    }
                } else {
        ...
        if (interaction.customId === 'close_ticket') {
            try {
                ...
                try {
                    ...
                    console.error('Error enviando respuesta de error de cierre:', replyError);
                }
            }
            return;
        }
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId.startsWith('ticket_modal_')) {
            ...
            // Defer inmediato — tenemos 3s antes de que Discord invalide la interacción
            try {
                await interaction.deferReply({ ephemeral: true });
            } catch (err) {
                console.error('[TICKETS] No se pudo hacer deferReply en ticket_modal:', err);
            }

            const safeEditReply = async (options) => {
                try {
                    // ephemeral no es válido en editReply, solo el contenido
                    const { ephemeral, ...rest } = options;
                    return await interaction.editReply(rest);
                } catch (e) {
                    console.error('[TICKETS] Error en editReply del modal:', e);
                }
            };
            ...
            if (maxTickets > 0) {
                ...
                if (userTickets.size >= maxTickets) {
                    console.warn(`[TICKETS] El usuario ${user.tag} ya tiene el máximo de tickets abiertos (${userTickets.size}/${maxTickets}).`);
                    return safeEditReply({ content: `❌ Ya tienes el límite máximo de tickets abiertos en este servidor (${maxTickets}). Por favor, cierra tus tickets anteriores antes de abrir uno nuevo.` });
                }
            }
            ...
            try {
                ...
                console.log('[TICKETS] ticket_modal canal creado:', channel.name);
                ...
                await safeEditReply({ content: `✅ Tu ticket ha sido creado: ${channel}` });
                ...
                return safeEditReply({ content: '❌ No se pudo crear el ticket. ' + (error.code === 50035 ? 'La categoría está llena.' : 'Revisa los permisos del bot.') });
            }
        }
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        if (interaction.customId === 'logs_select_channel') {
            ...
            // 2. Sincronizar con logs para el panel web
